...
const AllBookings = () => {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const res = await axios.get(`${window.location.origin}/api/bookings`, {
        params: {
  ...
  const handleUpdateStatus = async (id) => {
    const result = await Swal.fire({
      ...
      text: "Are you sure you want to update the status of this booking?",
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        const res = await axios.patch(
          `${window.location.origin}/api/bookings/${id}`,
          {
            role: user.role,
            status: "completed",
